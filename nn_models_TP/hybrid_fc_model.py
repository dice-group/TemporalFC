...
class HybridFCModel(BaseKGE):
    ...
    def __init__(self, args):
        ...
        if isinstance(self.ent_embeddings, pd.DataFrame):
            print("dataframe")
    ...
    def forward_triples(self, e1_idx, rel_idx, e2_idx,t_data,sen_idx, v_data, type="training"):
        # print(sen_idx)
        ...
        if type.__contains__("training"):
            ...
            ver_score = self.copaal_veracity_score_train(v_data)
        elif type.__contains__("valid"):
            emb_sen = self.sentence_embeddings_valid(sen_idx)
            ver_score = self.copaal_veracity_score_valid(v_data)
        else:
            emb_sen = self.sentence_embeddings_test(sen_idx)
            ver_score = self.copaal_veracity_score_test(v_data)

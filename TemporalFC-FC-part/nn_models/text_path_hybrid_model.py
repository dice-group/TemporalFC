...
class TextPathHybridModel(BaseKGE):

    def __init__(self, args):
        super().__init__()
        self.name = 'TextPathHybridModel'
    ...
    def forward_triples(self, e1_idx, rel_idx, e2_idx, x_data="",  type="training"):
        ...
        if type.__contains__("training"):
            emb_sen = self.sentence_embeddings_train(x_data)
            ver_score = self.copaal_veracity_score_train(x_data)
        elif type.__contains__("valid"):
            emb_sen = self.sentence_embeddings_valid(x_data)
            ver_score = self.copaal_veracity_score_valid(x_data)
        else:
            emb_sen = self.sentence_embeddings_test(x_data)
        ...
        z = torch.cat([sentence_embedding,path_layer],1)

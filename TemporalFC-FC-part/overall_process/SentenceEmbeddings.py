        ...
        with open(data_dir+self.data_file, "r") as file1:
            for line in file1:
                ...
                if line.startswith(' defactoScore'):
                    ...
                    if p[-1]=="_":
                        p = p[:-1]
                    if self.data_file.lower().__contains__("factbench") or self.data_file.lower().__contains__("bpdp"):
                        s, p, o = self.extractURI(self, data_dir=ddr, sub=s, pred=p, obj=o)
                        s, p, o = unquote(s), unquote(p), unquote(o)
                    else:
                        s, p, o = unquote(s), unquote(p), unquote(o)
            ...
            if web_ids == False:
                ...
                if (s+' '+p+' '+o) not in true_statements_embeddings.keys():
                    if bpdp == True:
                        ...
                        temp33 = temp[:3]
                            # select_top_n_sentences(temp2, n, temp, type.replace("_test",""),type)
...
if __name__ == '__main__':
    ...
    for d in datasets:
        if d=="factbench":
            ...
            path_dataset_folder = '../dataset/'
            # continue
            se = SentenceEmbeddings(data_dir=path_dataset_folder,multiclass=False, multiprops=property_split, dataset_file=dataset_file, copaal=full_hybrid)
        elif d =="bpdp":
            dataset_file = "BPDP_factcheckoutput.txt"
            path_dataset_folder = '../dataset/'
            continue
            se = SentenceEmbeddings(data_dir=path_dataset_folder, multiclass=False, multiprops=property_split, dataset_file=dataset_file, bpdp =True)
        else:
            dataset_file = d+"_factcheckoutput.txt"
            se = SentenceEmbeddings(data_dir=path_dataset_folder, multiclass=False, multiprops=property_split, dataset_file=dataset_file, bpdp=False)

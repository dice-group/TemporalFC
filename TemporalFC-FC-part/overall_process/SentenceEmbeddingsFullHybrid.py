...
class SentenceEmbeddings:
    def __init__(self, data_dir=None, multiclass=True, multiprops = False, dataset_file = "", bpdp = False, fullhybrid = False):
        ...
        if bpdp:
            self.extract_sentence_embeddings_from_factcheck_output_bpdp(self, data_dir,full_hybrid= fullhybrid)
        elif multiclass:
            self.extract_sentence_embeddings_from_factcheck_output_multiclass(self, data_dir, full_hybrid= fullhybrid)
        elif multiprops:
    ...
    @staticmethod
    def extract_sentence_embeddings_from_factcheck_output_bpdp(self, data_dir=None, get_website_ids=False, full_hybrid= False):
        data_train = []
        data_test = []
        pred = []
        test = False
            ...
            for line in file1:
                ...
                if line.startswith(' defactoScore'):
                    ...
                    if test == True and train == False:
                        ...
                        data_test.append([s, p, o, correct, sentences, trustworthiness, score])

                    if test == False and train == True:
                        print("train" + str(correct))
                        data_train.append([s, p, o, correct, sentences, trustworthiness, score])
        ...
        embeddings_textual_evedences_train_date, true_statements_embeddings = self.getSentenceEmbeddings(self, data_dir,
                                                                                                         ...
                                                                                                         true_statements_embeddings,
                                                                                                         'bpdp_complete', n,
                                                                                                         False, bpdp=True, web_ids=get_website_ids, full_hybrid=full_hybrid)
        ...
        embeddings_textual_evedences_test_date, true_statements_embeddings_test = self.getSentenceEmbeddings(self,
                                                                                                             ...
                                                                                                             true_statements_embeddings_test,
                                                                                                             'bpdp_complete_test',
                                                                                                             n, False, bpdp=True, web_ids=get_website_ids, full_hybrid=full_hybrid)
    ...
    @staticmethod
    def getSentenceEmbeddings(self, data_dir, data, model, true_statements_embeddings, type = 'default', n =3, props = False, bpdp = False, web_ids = False, full_hybrid=False):
        ...
        if web_ids == True:
            ...
            if bpdp == True:
                path = ""
                if full_hybrid:
                    data_dir = data_dir+"data/bpdp/data/copaal/"
    ...
    @staticmethod
    def extract_sentence_embeddings_from_factcheck_output_multiclass(self, data_dir=None, full_hybrid = False):
        data_train = []
        data_test = []
            ...
            for line in file1:
                ...
                if line.startswith(' defactoScore'):
                    ...
                    if train == True:
                        file_train = "train"
                    file_cat = "domain/"
                    if neg_data_dir != "true":
                        file_cat = neg_data_dir.replace("wrong/","")


                    ver = self.getVeracityScore(self, data_dir=data_dir+"data/copaal/"+file_train+"/"+file_cat+file_train, sub=s, pred=p, obj=o)
                    sentences = []
                    websites = []
                    trustworthiness = []
                    ...
                    if test == True and train == False:
                        print("test" + str(correct))
                        data_test.append([s, p, o, correct, sentences,trustworthiness,ver])

                    if test == False and train == True:
                        print("train" + str(correct))
                        data_train.append([s, p, o, correct, sentences,trustworthiness,ver])

                    if multiclass_neg_exp == True:
                        if test == False and train == True:
                            if neg_data_dir.__contains__("/date/") or neg_data_dir.__contains__("true"):
                                date_data_train.append([s, p, o, correct, sentences,trustworthiness,ver])
                            if neg_data_dir.__contains__("/domain/") or neg_data_dir.__contains__("true"):
                                domain_data_train.append([s, p, o, correct, sentences,trustworthiness,ver])
                            if neg_data_dir.__contains__("/domainrange/") or neg_data_dir.__contains__("true"):
                                domainrange_data_train.append([s, p, o, correct, sentences,trustworthiness,ver])
                            if neg_data_dir.__contains__("/mix/") or neg_data_dir.__contains__("true"):
                                mix_data_train.append([s, p, o, correct, sentences,trustworthiness,ver])
                            if neg_data_dir.__contains__("/property/") or neg_data_dir.__contains__("true"):
                                property_data_train.append([s, p, o, correct, sentences,trustworthiness,ver])
                            if neg_data_dir.__contains__("/random/") or neg_data_dir.__contains__("true"):
                                random_data_train.append([s, p, o, correct, sentences,trustworthiness,ver])
                            if neg_data_dir.__contains__("/range/") or neg_data_dir.__contains__("true"):
                                range_data_train.append([s, p, o, correct, sentences,trustworthiness,ver])

                        if test == True and train == False:
                            if neg_data_dir.__contains__("/date/") or neg_data_dir.__contains__("true"):
                                date_data_test.append([s, p, o, correct, sentences,trustworthiness,ver])
                            if neg_data_dir.__contains__("/domain/") or neg_data_dir.__contains__("true"):
                                domain_data_test.append([s, p, o, correct, sentences,trustworthiness,ver])
                            if neg_data_dir.__contains__("/domainrange/") or neg_data_dir.__contains__("true"):
                                domainrange_data_test.append([s, p, o, correct, sentences,trustworthiness,ver])
                            if neg_data_dir.__contains__("/mix/") or neg_data_dir.__contains__("true"):
                                mix_data_test.append([s, p, o, correct, sentences,trustworthiness,ver])
                            if neg_data_dir.__contains__("/property/") or neg_data_dir.__contains__("true"):
                                property_data_test.append([s, p, o, correct, sentences,trustworthiness,ver])
                            if neg_data_dir.__contains__("/random/") or neg_data_dir.__contains__("true"):
                                random_data_test.append([s, p, o, correct, sentences,trustworthiness,ver])
                            if neg_data_dir.__contains__("/range/") or neg_data_dir.__contains__("true"):
                                range_data_test.append([s, p, o, correct, sentences,trustworthiness,ver])
        ...
        n = 3
        # ///////////////////////////////////////////////////////////////////////////////train
        embeddings_textual_evedences_train_date, true_statements_embeddings = self.getSentenceEmbeddings(self, data_dir, date_data_train, model, true_statements_embeddings, 'date', n, full_hybrid=full_hybrid)

        embeddings_textual_evedences_train_domain, true_statements_embeddings = self.getSentenceEmbeddings(self, data_dir, domain_data_train, model, true_statements_embeddings, 'domain', n, full_hybrid=full_hybrid)

        embeddings_textual_evedences_train_domainrange, true_statements_embeddings = self.getSentenceEmbeddings(self, data_dir, domainrange_data_train, model, true_statements_embeddings, 'domainrange', n, full_hybrid=full_hybrid)

        embeddings_textual_evedences_train_mix, true_statements_embeddings = self.getSentenceEmbeddings(self, data_dir, mix_data_train, model, true_statements_embeddings, 'mix', n, full_hybrid=full_hybrid)

        embeddings_textual_evedences_train_property, true_statements_embeddings = self.getSentenceEmbeddings(self, data_dir, property_data_train, model, true_statements_embeddings, 'property', n, full_hybrid=full_hybrid)

        embeddings_textual_evedences_train_random,true_statements_embeddings = self.getSentenceEmbeddings(self, data_dir, random_data_train, model, true_statements_embeddings, 'random', n, full_hybrid=full_hybrid)

        embeddings_textual_evedences_train_range,true_statements_embeddings = self.getSentenceEmbeddings(self, data_dir, range_data_train, model, true_statements_embeddings, 'range', n, full_hybrid=full_hybrid)

        true_statements_embeddings_test = {}
        embeddings_textual_evedences_test_date, true_statements_embeddings_test = self.getSentenceEmbeddings(self, data_dir, date_data_test, model, true_statements_embeddings_test, 'date_test', n, full_hybrid=full_hybrid)
        embeddings_textual_evedences_test_domain,true_statements_embeddings_test = self.getSentenceEmbeddings(self, data_dir, domain_data_test, model, true_statements_embeddings_test, 'domain_test', n, full_hybrid=full_hybrid)
        embeddings_textual_evedences_test_domainrange, true_statements_embeddings_test = self.getSentenceEmbeddings(self, data_dir, domainrange_data_test, model, true_statements_embeddings_test, 'domainrange_test', n, full_hybrid=full_hybrid)
        embeddings_textual_evedences_test_mix,true_statements_embeddings_test = self.getSentenceEmbeddings(self, data_dir, mix_data_test, model, true_statements_embeddings_test, 'mix_test', n, full_hybrid=full_hybrid)
        embeddings_textual_evedences_test_property,true_statements_embeddings_test = self.getSentenceEmbeddings(self, data_dir, property_data_test, model, true_statements_embeddings_test, 'property_test', n, full_hybrid=full_hybrid)
        embeddings_textual_evedences_test_random,true_statements_embeddings_test = self.getSentenceEmbeddings(self, data_dir, random_data_test, model, true_statements_embeddings_test, 'random_test', n, full_hybrid=full_hybrid)
        embeddings_textual_evedences_test_range,true_statements_embeddings_test = self.getSentenceEmbeddings(self, data_dir, range_data_test, model, true_statements_embeddings_test, 'range_test', n, full_hybrid=full_hybrid)
...
for d in datasets:
    if d=="factbench":
        ...
        path_dataset_folder = '../dataset/'
        continue
        se = SentenceEmbeddings(data_dir=path_dataset_folder, multiclass=mc, multiprops=property_split, dataset_file=dataset_file, fullhybrid=full_hybrid)
    elif d =="bpdp":
        dataset_file = "BPDP_factcheckoutput.txt"
        path_dataset_folder = '../dataset/'
        # continue
        se = SentenceEmbeddings(data_dir=path_dataset_folder, multiclass=mc, multiprops=property_split, dataset_file=dataset_file, bpdp =True, fullhybrid=full_hybrid)

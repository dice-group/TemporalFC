...
'''
class SentencesExtractionEmbeddingStoring:
    ...
    def extract_sentence_embeddings_from_factcheck_output_DBpedia34k_dataset(self, data_dir=None):
        ...
        data_test = []
        print(data_dir)
        # ds_types = ["positive_train/", "positive_test/", "negative_test/", "negative_train/"]
        ds_types = ["positive_train/","positive_test/"]
        # "positive_train/", "positive_test/", "negative_test/", "negative_train/"
        # ds_types = ["negative_train/", "negative_test/"]
        for typ in ds_types:
            data_path = data_dir + typ+ self.data_file
                ...
                with open(data_path, "r") as file1:
                    for line in file1:
                        if line.startswith("{\"taskid\":\""):
                            ...
                            elif typ.__contains__("train"):
                                print("train" + str(correct))
                                data_train.append(
                                    [sub, pred, obj, correct, top_three_sentences, top_three_sentences_trust_scores,
                                     defacto_scores])
                            else:
                                print("type could be train/test only")
                                exit(1)
            else:
        ...
        n = 3
        self.getSentenceEmbeddings(self, data_dir,data_test,model,"test", n,False, bpdp=False)


        self.getSentenceEmbeddings(self,data_dir,data_train,model,"train",n, False, bpdp=False)
    ...
    def saveSentenceEmbeddingToFile(self, path, data2 , type2,n):
        ...
        for test in data2:
            if firstline:
                for j in range(len(list(test.values())[0])):
                    firstline_numbers += str(j+3) +"\t"
                line_dict["0\t1\t2"]= firstline_numbers
                # header.append(np.concatenate((list(line_dict.keys())[0].split('\t'), (np.array(list(str(line_dict.values())[14:-5].split('\\t')))).flatten()),axis=0))
            else:
                header.append(
                    np.concatenate((list(test.keys())[0].split('	'), (np.array(list(test.values()))).flatten()),
                                   axis=0))
            firstline= False
            # print(header)
            # header.append(test.values())
        #     maybe needed later so commenting it
        # for i in range(len(header)):
        #     header[i][0]=str.encode(header[i][0])
        #     header[i][1] = str.encode(header[i][1])
        #     header[i][2] = str.encode(header[i][2])
        X = pd.DataFrame(header)
        compression_opts = dict(method='zip', archive_name=type2+'SE.csv')
        X.to_csv(path + type2+'SE.zip', index=False, compression=compression_opts, sep='\t')
    ...
    @staticmethod
    def getSentenceEmbeddings(self, data_dir, data, model, type = 'default', n =3, props = False, bpdp = False):
        ...
        path = ""
        if type.__contains__("test"):
            path = data_dir + "hybrid_data/test/"
            isExist = os.path.exists(path)
            if not isExist:
                os.makedirs(path)
                print("The new directory is created!")
            self.saveSentenceEmbeddingToFile(self, path, embeddings_textual_evedences, "test", n)
        elif type.__contains__("train"):
            path = data_dir + "hybrid_data/train/"
            isExist = os.path.exists(path)
            if not isExist:
                os.makedirs(path)
                print("The new directory is created!")
...
if __name__ == '__main__':
    ...
    if args.eval_dataset == "Dbpedia34k":
        se = SentencesExtractionEmbeddingStoring(data_dir=args.path_dataset_folder)

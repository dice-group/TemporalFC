...
def argparse_default(description=None):
    ...
    parser.add_argument("--dataset", type=str, default="WTR",
                        help = "Available datasets: DBpedia54k, Yago3k, WTR ")
...
class GenerateTrainTestTriplesSet:
    ...
    def __init__(self, data_dir=None, multiclass_factbench=True, dataset_file ="", bpdp = False, copaal = False):
    ...
    @staticmethod
    def extract_sentence_embeddings_from_factcheck_output_bpdp(self, data_dir):
        data_train = []
        data_test = []
        final_output = []
    ...
    def extract_sentence_embeddings_from_factcheck_output(self,data_dir):
        ...
        with open(data_dir+ self.dataset_file.split("_")[0]+ "/train.txt", "w") as prediction_file:
            new_line = ""
            for idx, (head, relation, tail, lbl) in enumerate(data_train):
                new_line += head + "\t" + relation + "\t" + tail + "\t" + str(lbl) + "\n"
        ...
        with open(data_dir+ self.dataset_file.split("_")[0]+ "/test.txt", "w") as prediction_file:
            new_line = ""
            for idx, (head, relation, tail, lbl) in enumerate(data_test):
                new_line += head + "\t" + relation + "\t" + tail + "\t" + str(lbl) + "\n"
    ...
    @staticmethod
    def extract_sentence_embeddings_from_factcheck_output_multiclass(self, data_dir, copaal = False):
        data_train = []
        data_test = []
        date_data_train = []
        date_data_test = []
...
if __name__ == '__main__':
    ...
    for d in datasets:
        if d=="factbench":
            ...
            # continue
            se = GenerateTrainTestTriplesSet(data_dir=path_dataset_folder,multiclass_factbench=True, dataset_file=dataset_file, copaal=full_hybrid)
        elif d =="bpdp":
            dataset_file = "BPDP_factcheckoutput.txt"
            # continue
            se = GenerateTrainTestTriplesSet(data_dir=path_dataset_folder, multiclass_factbench=False, dataset_file=dataset_file, bpdp =True, copaal=full_hybrid)

        else:
            dataset_file = d+"_factcheckoutput.txt"
            # continue
            se = GenerateTrainTestTriplesSet(data_dir=path_dataset_folder, multiclass_factbench=False, dataset_file=dataset_file, bpdp=False, copaal=full_hybrid)
